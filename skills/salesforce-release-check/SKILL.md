...
6. `security` と `source.unpackaged` / `source.approval_processes` を見て permission / unpackaged metadata の整合を確認する。
7. `quality_gates.smoke_tests` があれば release 前 smoke として確認する。
8. `quality_gates.blocking_rules` と `quality_gates.static_analysis.blocking_rules` を満たしているかで `PASS / FAIL / CONDITIONAL` を判定する。

## 出力

- `PASS`: blocking rule を満たし、release 可能
- `FAIL`: release を止めるべき
- `CONDITIONAL`: 手動確認や運用判断付きでのみ進められる
- `Critical / Warning / Advisory` の指摘一覧
